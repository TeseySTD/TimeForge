...
const Timers: React.FC = () => {
    const [timersSets, setTimersSets] = useState<TimersSet[]>(() => initData());
    const [selectedTimersSet, setSelectedTimersSet] = useState<TimersSet | undefined>(() =>
        (() => {
            const d = initData();
            return d.length > 0 ? d[0] : undefined;
        })()
    );
    const [selectedTimer, setSelectedTimer] = useState<Timer | undefined>(() =>
        selectedTimersSet?.timers[0] ?? undefined
    );
